...
local Default = { -- Base settings
  -- =string.match(GetWorldName(),"%a+"), -- World
  Sell = false,
  Transfer = false,
  Deposit = false,
  Launder = false,
...
local Localisation = {
  en = {
    ...
    LowMaterial = {
      entry = "Low level materials",
      tooltip = "<150 level (except raw materials)",
    },
    ...
    StyleMaterial = {
      ...
      [3] = " (except rare style materials)",
    },
    TraitMaterial = {
      entry = "Trait material",
      [3] = " (except jewelry materials)",
    },
    CraftingBooster = {entry = "Crafting boosters"},
    Ingridient    = {
      entry = "Food ingredients",
      [3] = " (except " .. ItemQuality[4] .. " quality)",
    },
      ...
      if not data.isJunk and not IsItemLinkStolen(itemLink) and not IsItemLinkCrafted(itemLink) then
        ...
        if itemId then
          ...
          if ConsumableItems[itemId] then
            table.insert(ConsumableItems[itemId].Info[sourceBag], {
            slotIndex = slotIndex,
          ...
          elseif MonsterParts[itemId] and Action == 5 and not SavedVars.MonsterParts then
            table.insert( QueueData, {
              ...
              stackCount,
              itemLink,
              (stackCount < stackMax and itemId or nil),
            })
      ...
      for _, data in pairs(Item.Info[BAG_BACKPACK]) do
        ...
        if count > 0 then
          ...
          table.insert(QueueData, {
            ...
            count,
            data.itemLink,
            (count < data.stackMax and id or nil),
          })
        end
      end
    end
      ...
      if type(Default[param]) == "boolean" then
        ...
        for b = 3, 6 do
          ...
          button:SetHandler("OnMouseEnter", function(self)
            ...
            ZO_Tooltips_ShowTextTooltip(self, 
              BOTTOM, 
              zo_strformat(
                Localisation[lang].Tooltip[b] .. ((b < 5 and Default[param][1]) and Localisation[lang].Tooltip[7] or "") .. ((b == 3 and Default[param][2]) and Localisation[lang].Tooltip[8] or "") .. ((Localisation[lang][param] and Localisation[lang][param][b]) or ""),
                ItemQuality[SavedVars[param][1]],
                SavedVars[param][2]))
          end)
          button:SetHandler("OnMouseExit", function(self)ZO_Tooltips_HideTextTooltip() self:SetColor(.6,.57,.46,1) end)
        ...
        if Default[param][1] ~= nil then
          ...
          comboBox:SetFont("ZoFontGame")
          scroll.combobox[#scroll.combobox + 1] = {
            control = comboBox, 
            var = param,
          }
        ...
        if Default[param][2] ~= nil then
          ...
          scroll.editbox[#scroll.editbox + 1] = {
            control = control.eb, 
            var = param,
          }
